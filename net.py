...
class imagetoclass2(nn.Module):
    ...
    def cal_L(self, S, Q, task_index,special_l, mode, k):
        ...
        for i in range(self.n_class):
            ...
            if i + 1 in task_index:
                if mode == 'train':
                    num_samples = special_list.pop(0) // 2
                elif mode == 'test':
                    num_samples = special_list.pop(0)
            else:
                if mode == 'train':
                    num_samples = self.batch_size_per_class
                elif mode == 'test':
                    num_samples = self.sample_size
            Si = S[count:count + num_samples]  # Slice S to get the required samples
            count += num_samples
        ...
        for i in range(len(S_add_T_norm)):
            ...
            a = torch.matmul(Si, Q_norm)
            a, _ = torch.topk(a, k, 1)
...
class DM_MRN_train(nn.Module):
    ...
    def forward(self,support,query,task_index,special_list,iters):
        ...
        special_llist = special_list.copy()
        relation1_copy = []
        c = 0
        for i in range(self.N_CLASSES):
            if i + 1 in task_index:
                special_count = special_llist[c] // 2
                relation1_copy.extend([L1[i].unsqueeze(0)] * special_count)
                c += 1
            else:
                common_count = self.BATCH_SIZE_PER_CLASS
                relation1_copy.extend([L1[i].unsqueeze(0)] * common_count)
        ...
        for i in range(self.N_CLASSES):
            if i + 1 in task_index:
                special_count = special_llist[c] // 2
                relation2_copy.extend([L1[i].unsqueeze(0)] * special_count)
                c += 1
            else:
                common_count = self.BATCH_SIZE_PER_CLASS
                relation2_copy.extend([L1[i].unsqueeze(0)] * common_count)
